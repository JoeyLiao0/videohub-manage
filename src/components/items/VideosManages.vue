        ...
        <div class="video-table">
            ...
            <div class="video-table-pages-button">
                <el-pagination v-model:current-page="queryInfo.page" v-model:page-size="queryInfo.limit"
                    :page-sizes="[6]" background="true" layout="total, sizes, prev, pager, next, jumper"
                    :total="total_videos*6" @current-change="handleCurrentChange" />
            </div>
        </div>
        <!-- 视频播放对话框 -->
...
async function getVideoList() {
    try {
        ...
        if (response != null && response.data.code === 200) {
            ...
            total_videos.value = response.data.data.pages.total_pages;
        }
    } catch (error) {
        if (error.message === "AUTHENTICATION_FAILED") {
            console.log("访问令牌失效，请重新登录");
...
const handleCurrentChange = (val) => {
    if (total_videos.value >= val && val >= 1) {
